...
void
getInfo(const std::string &key,
        redis::module::CuckooFilter<RedisInstance> &cuckooFilter,
        RedisInstance &redis)
{
    // Make sure the key does not exist.
    redis.del(key);
    ...
    cuckooFilter.reserve(key, 100, 10, false);

    // Prepare the output buffer.
    std::unordered_map<std::string, sw::redis::OptionalLongLong> output;
    cuckooFilter.info(key, output);
...
int main() {
    ...
    getInfo<RedisInstance>("some_test_bloom_filter", bloomFilter, redis);

    // Create a CuckooFilter object.
    redis::module::CuckooFilter<RedisInstance> cuckooFilter(redis);
    getInfo<RedisInstance>("some_test_cuckoo_filter", cuckooFilter, redis);
